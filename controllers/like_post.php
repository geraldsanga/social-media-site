    ...
    if(isset($_GET["post_id"])){
        ...
        if($num_rows > 0) { // If he has interacted with the post already
            while($row = $like_results->fetch_assoc()){
                ...
                $mysqli->query("UPDATE PostLike SET active = 1, inactive = 0 WHERE user_id='$user_id' AND post_id='$post_id'") or die($mysqli->error);
                header("location: ../views/post_details.php?post_id=$post_id");
                }
            }
        }else{ // If he hasn't interacted with the post already, create a like object
            $mysqli->query("INSERT INTO PostLike (user_id, post_id, active, inactive) VALUES ('$user_id', '$post_id', '$active', '$inactive')") or die($mysqli->error);
            header("location: ../views/post_details.php?post_id=$post_id");
        }
    }
?>
